...
import { verifyExtensionToken } from "./utils/auth.js";
// =============================================================
// ⚙️ CONFIGURATION
// =============================================================
...
(async () => {
  const res = await verifyExtensionToken();
  if (!res.success || !res.active) {
    console.warn("DopeGuard disabled — no subscription");
    return;
  }

  console.log("DopeGuard enabled for:", res.user.email);
  ...
  function shouldActivate() {
    ...
    if (
      riskyDomains.some((word) => new RegExp(`\\b${word}\\b`, "i").test(url))
    ) {
      return true;
    }
  ...
  function instantHeuristicBlock(el) {
    ...
    const textAround =
      el.closest("a,div,figure")?.innerText?.toLowerCase() || "";
